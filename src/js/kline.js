    ...
    if (data.length > 0) {
        ...
        if (GetStartTime(lastitemtime) == GetStartTime(sysdatetime)) { //当天
            ...
            for (t = lastitemtime + 60000; t <= GetEndTime(lastitemtime) ; t = t + 60000) {
                result.push([t, lastitemprice, null]);
            }
        }
            ...
            formatter: function () {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (this.x == data[i].Date * 1000) {
                        zde = parseFloat(data[i].ClosingPrice - data[i].OpeningPrice).toFixed(intFormatFloat);
                ...
                close = this.points[0].point.close.toFixed(intFormatFloat);
